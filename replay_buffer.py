...
class ReplayMemory:
    ...
    def get_num_episodes(self, reduce_buf = False):
        ...
        self.numpy_buffer = np.array(self.buffer, dtype=object)
    ...
    def get_stats(self):
        ...
        next_state_mins = next_state.min(0)
        next_state_maxes = next_state.max(0)
        state_mins = state.min(0)
        state_maxes = state.max(0)
        action_mins = action.min(0)
        action_maxes = action.max(0)
        next_state_means = next_state.mean(0)
        next_state_stds = next_state.std(0)
        ...
        return (state_mins, state_maxes, action_mins, action_maxes, 
            state_means, state_stds, action_means, action_stds, next_state_mins, 
            next_state_maxes, next_state_means, next_state_stds)
...
def load_mem_uncertain(args, memory, store_dir, dataset='train'):
    if dataset == 'train':
        filename = 'train_buffer.pkl'
    elif dataset =='test':
        filename = 'test_buffer.pkl'
    else:
        filename = 'oracle_buffer.pkl'
    buffer_dir = f'./datasets/mujoco/{args.env}'
