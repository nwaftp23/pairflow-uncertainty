...
class ConditionalDiagonalNormal(Distribution):
    ...
    def _compute_params(self, context, kwargs=None):
        ...
        if kwargs == None:
            ...
            params = self._context_encoder(context, **kwargs)
    ...
    def _log_prob(self, inputs, context, kwargs = None):
        ...
        # Compute parameters.
        means, log_stds = self._compute_params(context, kwargs=kwargs)
    ...
    def _sample(self, num_samples, context, kwargs = None):
        ...
        return torchutils.split_leading_dim(samples, [context_size, num_samples]), means, stds
...
class DiagonalNormal(Distribution):
    ...
    def _sample(self, num_samples, context, kwargs=None):
        stds = torch.exp(self.log_std_)
        means = torchutils.repeat_rows(self.mean_, num_samples)
        stds = torchutils.repeat_rows(stds, num_samples)

        # Generate samples.
        noise = torch.randn(num_samples, *self._shape, device=means.device)
        samples = means + stds * noise
        return samples, self.mean_, torch.exp(self.log_std_)
