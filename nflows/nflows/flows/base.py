...
class Flow(Distribution):
    ...
    def sample_and_log_prob(self, num_samples, context=None, kwargs=None, 
        ...
        if ensemble:
            ...
            #old_v = torch.log((torch.stack(base_probs)).mean(0))
            #old_v = old_v.reshape(nflows_log_prob.shape) 
            base_log_prob = torch.log(torch.stack(base_probs)).reshape(
                ensemble_size, num_context, num_samples)
            nflows_log_prob = torch.log(torch.stack(out_probs)).reshape(
                ensemble_size, num_context, num_samples)
            #out_log_probs = torch.log((torch.stack(out_probs)).mean(0))
            #base_log_prob = torch.log((torch.stack(base_probs)).mean(0))
            #nflows_log_prob = out_log_probs.reshape(nflows_log_prob.shape)
            #base_log_prob = base_log_prob.reshape(nflows_log_prob.shape) 
            #new_v = base_log_prob
            #new_v = torch.log(torch.exp(new_v).mean(0))
            #base_log_prob = old_v
            #print(old_v)
